...
export default function Dashboard() {
    ...
    // All users with task status counts
    ...
    // Fetch all users + their task status counts
    useEffect(() => {
        const fetchUsersTasks = async () => {
            setUsersLoading(true);
            try {
                const res = await fetch(`http://localhost:8181/api/v1/dashboard/users-tasks`, {
                    headers: { Authorization: `Bearer ${token}` }
                });
                if (!res.ok) throw new Error("Failed to fetch users task summary");
                const data = await res.json();
                setUsersTasks(data);
            } catch (err) {
                console.error(err);
    ...
    // Badge component for task status
            ...
            default:
                return loading ? (
                    ...
                    <div className="space-y-6">
                        ...
                        {/* Users Task Table - Admin/CEO Only */}
                        {(user.roles?.includes("ADMIN") || user.roles?.includes("CEO")) && (
                            <div className="bg-white border border-gray-300 p-4 rounded-md shadow-sm">
                                <h2 className="text-lg font-semibold text-gray-700 mb-4">Users Task Summary</h2>

                                {usersLoading ? (
                                    <p>Loading users...</p>
                                ) : (
                                    <div className="overflow-x-auto">
                                        <table className="w-full text-sm text-left text-gray-700 table-auto min-w-max">
                                            ...
                                            {usersTasks.map((u) => {
                                                ...
                                                return (
                                                    <tr key={u.userId} className="hover:bg-gray-50">
                                                        ...
                                                        <td className="px-3 py-2">{u.roles?.join(", ") || "N/A"}</td>
                                                        <td className="px-3 py-2"><StatusBadge status="TODO" count={tasks.TODO} /></td>
                                                        <td className="px-3 py-2"><StatusBadge status="IN_PROGRESS" count={tasks.IN_PROGRESS} /></td>
                                                        <td className="px-3 py-2"><StatusBadge status="SUBMITTED" count={tasks.SUBMITTED} /></td>
                                                        <td className="px-3 py-2"><StatusBadge status="APPROVED" count={tasks.APPROVED} /></td>
                                                        <td className="px-3 py-2"><StatusBadge status="REJECTED" count={tasks.REJECTED} /></td>
